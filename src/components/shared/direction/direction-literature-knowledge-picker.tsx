...
import { importKnowledgeToCorpus } from "@/services/direction-literature";

interface DirectionLiteratureKnowledgePickerProps {
  slug: string;
  onImported?: () => void;
}
...
/** 从知识库 PDF 加入 Direction 文献 corpus */
export function DirectionLiteratureKnowledgePicker({
  slug,
  onImported,
}: DirectionLiteratureKnowledgePickerProps) {
  ...
  const handleImport = useCallback(
    async (fileName: string) => {
      ...
      try {
        ...
        await importKnowledgeToCorpus(slug, fileName, citation, "supporting");
        toast.success("已加入文献 corpus");
        onImported?.();
      } catch (e) {
  ...
  return (
    <div className="space-y-2">
      ...
      {filtered.length === 0 ? (
        ...
        <ScrollArea className="max-h-[min(360px,45vh)]">
