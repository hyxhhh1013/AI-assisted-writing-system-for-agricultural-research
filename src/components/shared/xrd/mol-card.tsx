...
interface MolCardProps extends PlotToolProps {
  prefill?: PlotToolPrefill | null;
}

export function MolCard({ title: toolTitle, description, onInsertToPaper, prefill }: MolCardProps) {
  ...
  useEffect(() => {
    if (!prefill || prefill.figureId !== "molecule") return;
    const c = prefill.config;
    const nextMode = configString(c, "mode", "mol") === "rxn" ? "rxn" : "mol";
    setMode(nextMode);
    if (nextMode === "rxn") {
      setRxnSmiles(configString(c, "rxn_smiles", "CC(=O)O.CCO>>CC(=O)OCC"));
      setRxnLabel(configString(c, "rxn_label", ""));
    } else {
      setSmiles(configString(c, "smiles", "CC(=O)O"));
      setLabel(configString(c, "label", ""));
    }
    setResult(null);
  }, [prefill]);
  ...
  const buildReplayConfig = () =>
    mode === "mol"
      ? { mode: "mol" as const, smiles, label }
      : { mode: "rxn" as const, rxn_smiles: rxnSmiles, rxn_label: rxnLabel };
                  ...
                  <Button
                    ...
                    onClick={() =>
                      onInsertToPaper(
                        result.imageUrl,
                        caption,
                        buildPlotInsertReplay("molecule", caption, buildReplayConfig()),
                      )
                    }
                  >
