  ...
  return (
    <div className="flex h-screen bg-[#faf9f6]">
      <aside className="flex w-56 flex-col border-r border-[#1a5632]/10 bg-white">
        ...
        <nav className="flex-1 overflow-y-auto px-2 space-y-4">
          {NAV_GROUPS.map((group) => (
            <div key={group.label}>
              <p className="px-3 py-1 text-[10px] font-medium uppercase tracking-[0.15em] text-[#9aa8a0]">
                {group.label}
              </p>
      ...
      <main className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-12 shrink-0 items-center justify-between border-b border-[#1a5632]/10 bg-white px-6">
          <p className="text-xs text-[#9aa8a0] hidden sm:block">禾书耕文 · 后台管理</p>
          <AdminGlobalSearch />
        </header>
