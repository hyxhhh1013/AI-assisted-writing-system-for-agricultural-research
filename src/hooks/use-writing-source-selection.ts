...
export function useWritingSourceSelection(params: WritingSourceSelectionParams) {
  ...
  const fetchPreview = useCallback(async () => {
    ...
    try {
      ...
      // 综述模式：即使 0 结果也不能自动确认，必须用户显式确认
      setConfirmed(!reviewRequiresConfirmation && preview.hitCount === 0);
      if (preview.hitCount === 0) {
        if (reviewRequiresConfirmation) {
          toast.info("未检索到匹配文献，请确认后将按通用知识扩写");
        } else {
  ...
  const confirmSelection = useCallback(() => {
    ...
    if (reviewRequiresConfirmation && selectedSourceIds.length === 0) {
      toast.error("综述写作需要至少选择一篇文献作为参考");
      return false;
    }
    ...
    return true;
  }, [fetchedOnce, previewStale, hits.length, selectedSourceIds.length, reviewRequiresConfirmation]);
  ...
  const needsConfirmation = hits.length > 0 || fetchedOnce || reviewRequiresConfirmation;
