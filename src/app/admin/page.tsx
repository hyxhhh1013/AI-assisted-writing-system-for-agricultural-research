...
import { Users, FileText, Database, Search, Clock, Loader2 } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  if (!stats) {
    return (
      <div className="text-center text-sm text-[#6b7c72] py-20">
        加载失败，请确认管理员权限
      </div>
    );
  }
  ...
  const statCards = [
    ...
    { label: "查重检测", value: stats.plagiarismCount, icon: Search, color: "text-purple-500" },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {statCards.map((card) => (
          <div
            key={card.label}
            className="rounded-xl border border-[#1a5632]/10 bg-white p-4"
          >
            <div className="flex items-center gap-2 text-xs text-[#6b7c72]">
              <card.icon className={`h-4 w-4 ${card.color}`} />
              {card.label}
            </div>
            <p className="mt-2 text-2xl font-bold text-[#122820]">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-[#1a5632]/10 bg-white p-4">
          <h2 className="mb-3 text-xs font-medium text-[#6b7c72]">
            近 7 天新建项目
          </h2>
            ...
            {stats.projectTrend.map((day) => {
              ...
              return (
                <div key={day.date} className="flex flex-1 flex-col items-center gap-1">
                  ...
                  <div
                    className="w-full rounded-t bg-[#1a5632]/70 transition-all"
                    style={{ height: `${Math.max(h, 4)}%` }}
                  />
                  <span className="text-[9px] text-[#9aa8a0]">
                    {day.date.slice(5)}
                  </span>
                </div>
              );
            ...
            {stats.filesByCategory.slice(0, 6).map((cat) => {
              ...
              return (
                <div key={cat.category} className="flex items-center gap-2">
                  <span className="w-20 truncate text-xs text-[#3d4f46]">
                    {cat.category || "未分类"}
                  </span>
                  <div className="flex-1 h-4 rounded bg-[#1a5632]/8 overflow-hidden">
                    <div
                      className="h-full rounded bg-[#1a5632]/60"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <span className="text-[10px] text-[#9aa8a0] w-8 text-right">
                    {cat.count}
                  </span>
                </div>
              );
            ...
            {stats.projectsByTemplate.map((t) => {
              ...
              return (
                <div key={t.template} className="flex items-center gap-2">
                  <span className="w-20 text-xs text-[#3d4f46]">
                    {TEMPLATE_LABELS[t.template] ?? t.template}
                  </span>
                  <div className="flex-1 h-4 rounded bg-[#1a5632]/8 overflow-hidden">
                    <div
                      className="h-full rounded bg-blue-400/60"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <span className="text-[10px] text-[#9aa8a0] w-8 text-right">
        ...
        <div className="rounded-xl border border-[#1a5632]/10 bg-white p-4">
          ...
          <div className="space-y-3">
              ...
              <div key={i} className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs text-[#3d4f46]">
                    <span className="font-medium">{a.user}</span> 更新了{" "}
                    <span className="font-medium">{a.title || "未命名"}</span>
                  </p>
            ...
            {stats.recentActivity.length === 0 && (
              <p className="text-xs text-[#9aa8a0]">暂无活动</p>
            )}
          </div>
        </div>
      </div>
