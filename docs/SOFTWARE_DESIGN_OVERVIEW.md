...
flowchart TB
  subgraph presentation [表现层 Presentation]
    Pages["App Router 页面<br/>workbench / projects / knowledge ..."]
    Components["业务组件 shared/*"]
    Hooks["Hooks<br/>use-writing-stream / use-auto-save ..."]
  end

  subgraph gateway [网关层 Gateway]
    Proxy["proxy.ts<br/>JWT · 路由保护 · AI 限流"]
  end

  subgraph application [应用层 Application]
    API["api/* Route Handlers"]
    Services["services/* 用例编排"]
  end

  subgraph domain [领域层 Domain]
    Lib["lib/* RAG · AI · Prompt · 引用校验"]
    Contracts["contracts/* 共享类型契约"]
  end

  subgraph infrastructure [基础设施 Infrastructure]
    Prisma["Prisma ORM"]
    PG[(PostgreSQL)]
    FS["papers/ · data/index_*"]
    LLM["DeepSeek / 智谱 API"]
    Py["Python matplotlib 子进程"]
  end
...
| S8 | **质量：查重与降重** | 查重页 `/plagiarism`、审查 `/review` | `api/plagiarism/*`、`services/plagiarism-service.ts` | `PlagiarismCheck`、`PlagiarismMatch`、`RewriteSuggestion` |
| S9 | **质量：一致性 / 审查** | 工作台对话框、审查中心 | `api/consistency/*`、`api/review/*` | `ReviewCheck` 等 |
| S10 | **图表与 XRD** | 绘图页、XRD 实验室 | `api/chart`、`api/figures/registry`、`scripts/charts/` | 项目 `charts` JSON、图片文件 |
...
sequenceDiagram
  ...
  participant H as use-writing-stream
  participant P as proxy
  participant W as /api/writing
  participant R as localRAG
  participant A as callAI
  participant DB as PostgreSQL

  U->>H: 点击扩写
  H->>P: POST /api/writing (Cookie JWT)
  P->>P: 校验登录 · 限流 · 注入 x-user-id
  P->>W: 转发请求
  W->>R: 混合检索文献片段
  W->>W: 构建 EvidencePack
  W->>A: Writer 流式生成
  A-->>W: SSE stream_token
  W->>A: Verifier 非流式核查
  W->>A: Refiner 条件修正
  W->>W: citation-validator
  W-->>H: SSE pipeline_step / complete
  H->>U: 更新编辑器
  Note over U,DB: 保存由 use-auto-save → PATCH sections
```
...
下表将**代码中的惯用法**映射到经典 GoF / 架构模式名称，便于答辩表述。
...
| **BFF（Backend for Frontend）** | Next.js Route Handler 为前端定制聚合 API | 全部 `src/app/api/*` |
| **功能开关（Feature Toggle）** | 环境变量控制模块是否展示 | `feature-flags.ts` + `APP_MODULES` |
| **降级（Graceful Degradation）** | RAG 无结果仍写作；Verifier 模型不可用换 DeepSeek | `writing/route.ts`、`models.ts` |
...
erDiagram
  ...
  Project ||--o{ PlagiarismCheck : has
  PlagiarismCheck ||--o{ PlagiarismMatch : contains
...
1. 本文 **§2 分层** + **§3 子系统表**  
2. `docs/ARCHITECTURE.md` — 三个核心架构决策（多 Agent、RAG、引用验证）  
3. `docs/WORKFLOWS.md` — 三条端到端流程（写作 / 保存 / 查重）  
4. 可选：`docs/CODE_MAP.md` — 功能 → 文件索引  
