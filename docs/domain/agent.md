...
- **附件提取失败自动重试（2026-08-08）**：上传 xlsx 等附件后提取失败（extract_failed）时，`read_attachment` 会自动重试一次提取（`retryAttachmentExtraction`：文件仍在则重新解析），成功后返回内容；仍失败返回友好提示「可重新上传或稍后再读」。修复「上传 xlsx 显示未能解析且无法恢复」（根因：提取是后台异步，首次失败后无重试）。
