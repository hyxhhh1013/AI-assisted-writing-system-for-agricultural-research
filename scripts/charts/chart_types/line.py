    ...
    def plot(self, labels, datasets, config, output_path):
        ...
        for i, ds in enumerate(datasets):
            ...
            if numeric_x is not None:
                ax.plot(numeric_x, d, color=c, marker="o", linewidth=2, markersize=6,
                        label=lbl or None, zorder=3)
            else:
                x_pos = range(len(labels))
                ax.plot(x_pos, d, color=c, marker="o", linewidth=2, markersize=6,
                        label=lbl or None, zorder=3)

        if numeric_x is None:
            ax.set_xticks(range(len(labels)))
            ax.set_xticklabels(labels_display, fontsize=9, rotation=20, ha="right")
        else:
